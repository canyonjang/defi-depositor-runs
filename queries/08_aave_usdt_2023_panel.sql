-- External validity (cell 3): Aave V2 USDT reserve, March 2023 episode. t0 = 2023-03-09.
...
bal AS (SELECT owner, SUM(s) AS bal0 FROM (
          ...
          UNION ALL SELECT owner, -amt FROM wdr WHERE ts < TIMESTAMP '2023-03-09') GROUP BY owner),
runs AS (SELECT owner, MIN(ts) AS first_wd_time, SUM(amt) AS wd_win FROM wdr
         WHERE ts >= TIMESTAMP '2023-03-09' AND ts < TIMESTAMP '2023-03-15' GROUP BY owner),
soph AS (SELECT "from" AS owner, COUNT(*) AS tx_sent_count, MIN(block_time) AS first_tx_time
         FROM ethereum.transactions WHERE block_time < TIMESTAMP '2023-03-09'
